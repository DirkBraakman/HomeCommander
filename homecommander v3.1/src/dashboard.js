// Dashboard renderer: UI logic for HomeCommander
const { ipcRenderer } = require("electron");
const axios = require("axios");
const rotaryBrightnessStep = 10;
const rotaryHueStep = 10;
let config = null,
  haClient = null;
...
async function initDashboard() {
  ...
  // Listen for rotary events from main process
  ...
  require("electron").ipcRenderer.on(
    "button-control",
    async (event, { buttonIndex }) => {
      if (!config?.assignments?.[buttonIndex + 3]) return; // Button 1=4, 2=5, 3=6
      const assignment = config.assignments[buttonIndex + 3];
      if (assignment.property === "toggle") {
        await handleButtonToggle(buttonIndex + 3, assignment);
      }
    }
  );
}
...
async function handleRotaryColor(controlNumber, assignment, direction) {
  ...
  if (typeof colorHues[controlNumber] !== "number")
    colorHues[controlNumber] = 0;
  if (direction === "right")
    colorHues[controlNumber] = (colorHues[controlNumber] + step) % 360;
  else colorHues[controlNumber] = (colorHues[controlNumber] - step + 360) % 360;
...
// Toggle handler for button assignment (light/switch)
async function handleButtonToggle(controlNumber, assignment) {
  if (!haClient || !assignment.entityId) return;
  try {
    const { data } = await haClient.get(`/states/${assignment.entityId}`);
